...
const { listContacts, getContactById, addContact, removeContact, updateContact, connectToDb } = require('./contacts');
...
const invokeAction = ({ action, ...args }) =>
  handlers[action]
    ? handlers[action](args).then(data => console.log(data || '')).catch(err => console.warn(err))
    : startServer();

connectToDb(() => invokeAction(argv));
